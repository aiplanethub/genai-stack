...
class ChromaDB(BaseMemory):
    ...
    def _post_init(self, *args, **kwargs):
        ...
        self.lc_client = self.mediator.get_vectordb(index_name=config.collection_name)
        retriever = self.lc_client.as_retriever(**{k:v for k,v in config.retrieve_parameters.items()})
    ...
    def get_user_text(self):
        if self._is_chat_conversation_history_available():
            return extract_text(key='user', text=self.lc_client.get()['documents'][-1])
        else:
            return None
    
    def get_model_text(self):
        if self._is_chat_conversation_history_available():
            return extract_text(key='model', text=self.lc_client.get()['documents'][-1])
        else:
            return None
    
    def _is_chat_conversation_history_available(self) -> bool:
        if len(self.lc_client.get()['documents']):
            return True
        else:
            return False
