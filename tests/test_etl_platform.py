...
class TestEtl(unittest.TestCase):
    def setUp(self) -> None:
        self.etl_loader = LangchainETL.from_kwargs(
            name="PyPDFLoader", fields={"file_path": "/path/to/pdf"}
        )
        self.embedding = get_default_embeddings()
        self.chromadb = ChromaDB.from_kwargs()

        self.stack = Stack(etl = self.etl_loader, embedding = self.embedding, vectordb= self.chromadb, model = None)
        self.etl_platform = PrefectETLPlatform(platform_config= PrefectPlatformConfig(), stack=self.stack)

    def test_etl_platform(self):
        dir = Path("/home/samjoel/Dphi/datasets/data")
        for fp in list(dir.glob("*.pdf"))[:10]:
            self.etl_platform.handle_job(file_path = fp)
